...
export class DriveSync {
    constructor(dbPrefix = 'links_', folderPath = '/backup/links-library/', itemsPerChunk = 50) {
        this.basePath = folderPath;
        this.dbPrefix = dbPrefix;
        this.itemsPerChunk = itemsPerChunk;
        this.metaFile = dbPrefix + 'meta_v1.bin';
        this.groupPrefix = dbPrefix + 'group_v1_';
    }
    ...
    async saveChunks(playlists, categories, vaultKey, folderId) {
        console.log(`[Drive] Saving ${playlists.length} playlists in groups of ${this.itemsPerChunk}`);
        ...
        for (let i = 0; i < playlists.length; i += this.itemsPerChunk) {
            playlistGroups.push(playlists.slice(i, i + this.itemsPerChunk));
        }

        // 3. Upload/Update Groups
        for (let i = 0; i < playlistGroups.length; i++) {
            const hash = await this.calculateHash(playlistGroups[i]);
            groupHashes[i] = hash;
            ...
            if (needsUpload || !fileMap.has(fileName)) {
                const encrypted = await Security.encrypt(playlistGroups[i], vaultKey);
    ...
    async loadChunks(folderId, vaultKey) {
        ...
        if (metaId) {
            try {
                ...
                console.log(`[Drive] Loading v1 data. Groups: ${Object.keys(meta.groupHashes).length}`);
                const playlists = [];
                ...
                for (const idx of indices) {
                    ...
                    if (fileId) {
                        ...
                        const gPlaylists = await Security.decrypt(enc, vaultKey);
                        playlists.push(...gPlaylists);
                    }
                }
                return { playlists, categories: meta.categories };
            } catch (e) { console.error("[Drive] v1 Load failed", e); }
        }
