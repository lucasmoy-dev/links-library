...
export function openEditor(note = null) {
    ...
    if (window.innerWidth < 1280 || window.innerHeight < 960) {
        if (!dialogContent.classList.contains('fullscreen')) {
            document.getElementById('note-expand-btn').click();
        }
    } else {
        // Ensure it's not fullscreen if it's a new large screen session
